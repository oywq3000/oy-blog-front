export default {
  nav: {
    home: 'Home',
    articles: 'Articles',
    editor: 'Editor',
    profile: 'Profile',
    logout: 'Logout',
    about: 'About',
    agent: 'AI Assistant'
  },
  about: {
    title: 'About OY Blog',
    introTitle: 'Introduction',
    introText: 'OY Blog is a modern full-stack personal blog system based on Spring Boot 3 + Vue 3. It adopts a separation of frontend and backend architecture, aiming to provide developers with a fully functional, high-performance, and elegant blog solution. Whether as a personal tech blog or a full-stack learning case, it is an excellent choice.',
    techStackTitle: 'Tech Stack Overview',
    frontend: 'Frontend Tech',
    backend: 'Backend Tech',
    featuresTitle: 'Core Features',
    feature1: 'Immersive Reading: Markdown rendering, syntax highlighting, math formulas',
    feature2: 'Responsive Design: Perfectly adapted for Desktop, Tablet, Mobile',
    feature3: 'Powerful Search: Full-text search based on Elasticsearch',
    feature4: 'Community Interaction: Nested comments, likes/favorites, anti-spam',
    authorTitle: 'About Author',
    authorText: 'We are the OY Blog Team, a group of developers passionate about technology. Dedicated to building high-quality open-source projects and growing with the community. If you like this project, please give us a Star on GitHub!',
  },
  auth: {
    login: 'Login',
    welcomeBack: 'Welcome Back',
    createAccount: 'Create Account',
    enterDetails: 'Enter your details to access your account',
    joinCommunity: 'Join our community of developers',
    processing: 'Processing...',
    signIn: 'Sign In',
    signUp: 'Sign Up',
    fillAll: 'Please fill in all fields',
    welcomeBackExcl: 'Welcome Back!',
    accountCreated: 'Account Created!',
    email: 'Email Address',
    emailPlaceholder: "john{'@'}example.com",
    password: 'Password',
    passwordPlaceholder: 'Enter password',
    confirmPassword: 'Confirm Password',
    username: 'Username',
    usernamePlaceholder: 'Enter your username',
    forgotPassword: 'Forgot Password?',
    noAccount: "Don't have an account?",
    hasAccount: 'Already have an account?',
    registerNow: 'Register now',
    passwordsDoNotMatch: 'Passwords do not match',
    verifyCode: 'Verification Code',
    verifyCodePlaceholder: '6-digit code',
    sendCode: 'Send Code',
    resendInSeconds: 'Resend in {seconds}s',
    codeSent: 'Verification code sent, please check your email',
    codeRequired: 'Please enter the verification code',
    codeSendFailed: 'Failed to send verification code',
    loginSubtitle: 'Start your technical journey',
    registerSubtitle: 'Share knowledge, Connect the world',
    title: 'OY Blog',
    joinTitle: 'Join OY Blog',
    successTitle: 'Success',
    welcomeTitle: 'Welcome',
    backToLogin: 'Back to login',
    loginFailed: 'Login failed',
    registerFailed: 'Registration failed',
    unknownError: 'An error occurred',
    or: 'or',
    resetTitle: 'Reset Password',
    resetSubtitle: 'Verify your email and set a new password',
    resetPassword: 'Reset Password',
    newPassword: 'New Password',
    resetSuccess: 'Password reset successfully. Please sign in with your new password.',
    resetFailed: 'Password reset failed',
    invalidEmail: 'Please enter a valid email address',
    invalidCode: 'Verification code must be 6 digits',
    captcha: 'Captcha',
    captchaPlaceholder: 'Enter characters shown',
    captchaRefresh: 'Click image to refresh',
    captchaRequired: 'Please complete the captcha',
  },
  home: {
    title: 'OY Blog',
    subtitle: 'Exploring The Tech Frontier',
    tag: '<DEV_ZONE />',
    featured: 'Featured Story',
    loadingTitle: 'Loading...',
    loadingSummary: 'Fetching latest articles...',
    latestArticles: 'Latest Articles',
    popularArticles: 'Most Popular',
    columns: 'Featured Columns',
    columnArticles: '{count} articles',
    scrollLeft: 'Scroll left',
    scrollRight: 'Scroll right',
    readFull: 'Read Full Article',
    noArticles: 'No articles yet',
    loadMore: 'Load More',
    loadingMore: 'Loading…',
    loadMoreFailed: 'Failed to load, click to retry',
    loadedAll: 'All loaded',
    stats: {
      articles: 'Articles',
      views: 'Views',
      likes: 'Likes',
      tags: 'Tags',
    },
  },
  profile: {
    articles: 'Articles',
    likes: 'Likes',
    following: 'Following',
    editProfile: 'Edit Profile',
    favorites: 'Favorites',
    history: 'History',
    settings: 'Settings',
    noFavorites: 'No favorites yet.',
    noHistory: 'No history yet.',
    viewedOn: 'Viewed on',
    joined: 'Joined on',
    activity: 'Activity',
    heatmapTooltip: '{count} contributions on {date}',
    accountSecurity: 'Account Security',
    emailAddress: 'Email Address',
    verified: 'Verified',
    unverified: 'Unverified',
    sending: 'Sending...',
    resendIn: 'Resend in {seconds}s',
    sendVerification: 'Send Verification Email',
    verificationSent: 'A verification link has been sent to your email. Please check your inbox.',
    changePassword: 'Change Password',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    updatePassword: 'Update Password',
    passwordPlaceholder: 'Enter password',
    guest: 'Guest',
    bio: 'Bio',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
    appearance: 'Appearance',
    darkMode: 'Dark Mode',
    reducedMotion: 'Reduced Motion',
    notifications: 'Notifications',
    emailDigest: 'Email Digest',
    newComments: 'New Comments',
    confirmNewPassword: 'Confirm New Password',
    enterCurrentPassword: 'Enter current password',
    enterNewPassword: '8+ chars, uppercase, lowercase, number',
    confirmNewPasswordPlaceholder: 'Confirm new password',
    hide: 'Hide',
    show: 'Show',
    uppercase: 'Uppercase',
    lowercase: 'Lowercase',
    number: 'Number',
    minChars: '8+ Chars',
    profileInfo: 'Profile Information',
    avatarUrl: 'Avatar',
    enterAvatarUrl: 'Enter image URL',
    username: 'Username',
    enterUsername: 'Enter username',
    bioText: 'Bio', // Renamed to avoid duplicate 'bio' key
    enterBio: 'Tell us about yourself',
    updateSuccess: 'Profile updated successfully',
    unfavorite: 'Remove Favorite',
    unfavoriteSuccess: 'Removed from favorites',
    passwordUpdated: 'Password updated successfully',
    passwordUpdateFailed: 'Failed to update password',
    verificationEmailSent: 'Verification email sent, please check your inbox',
    avatarCropTitle: 'Adjust Avatar',
    avatarCropHint: 'Drag to reposition, scroll / pinch to zoom',
    avatarZoomIn: 'Zoom in',
    avatarZoomOut: 'Zoom out',
    avatarReset: 'Reset',
    avatarCancel: 'Cancel',
    avatarConfirm: 'Confirm',
    avatarUploading: 'Uploading…',
    avatarNotImage: 'Please choose an image file',
    avatarTooLarge: 'Image must be smaller than 10MB',
    avatarTooSmall: 'Image must be at least 200px on each side',
    avatarDecodeFailed: 'Failed to read image, please pick another one',
    avatarUpdated: 'Avatar updated',
  },
  userProfile: {
    publishedArticles: 'Published Articles',
    noArticles: 'No published articles yet',
    notFound: 'User not found',
  },
  verify: {
    verifying: 'Verifying your email...',
    success: 'Email verified successfully!',
    invalidToken: 'Invalid verification link. Token is missing.',
    failed: 'Verification failed. The link may have expired.',
    error: 'An unexpected error occurred.',
    redirecting: 'Redirecting to profile...',
  },
  errors: {
    network: 'Network error, please check your connection and try again',
    timeout: 'Request timed out, please try again later',
    http: 'Request failed (HTTP {status})',
    unknown: 'Something went wrong, please try again later',
  },
  hero: {
    slogan: 'Exploring the Tech Frontier',

    cta: 'Start Reading',
    start: 'Start',
  },
  common: {
    search: 'Search',
    searchPlaceholder: 'Search articles...',
    history: 'History',
    clear: 'Clear',
    loading: 'Loading...',
    saving: 'Saving...',
    saveChanges: 'Save Changes',
    filter: 'Filters',
    readMore: 'Read More',
    recommended: 'Recommended',
    switchToZh: 'Switch to Chinese',
    switchToEn: 'Switch to English',
    viewReplies: 'View {count} replies',
    collapse: 'Collapse',
    reply: 'Reply',
    commentHidden: 'Comment hidden',
    show: 'Show',
    notFound: 'Page Not Found',
    notFoundDesc: 'The page you are looking for doesn\'t exist or has been moved.',
    backHome: 'Back to Home',
    goBack: 'Go Back',
  },
  sidebar: {
    author: 'OY Coder',
    bio: 'Full Stack Developer passionate about building scalable systems and exploring new technologies.',
    hotTags: 'Hot Tags',
    defaultBio: 'This person is too cool to say anything!',
  },
  footer: {
    desc: 'Sharing knowledge and experience in the world of technology.',
    stayUpdated: 'Stay Updated',
    emailPlaceholder: 'Enter your email',
    subscribe: 'Subscribe',
    rights: '© 2023 OY Blog. All rights reserved.',
    privacy: 'Privacy Policy',
    terms: 'Terms of Service',
    product: 'Product',
    company: 'Company',
    resources: 'Resources',
    community: 'Community',
    helpCenter: 'Help Center',
  },
  editor: {
    titlePlaceholder: 'Please enter title',
    saving: 'Saving...',
    words: 'words',
    publish: 'Publish',
    saveDraft: 'Save Draft',
    back: 'Go Back',
    settings: 'Settings',
    summary: 'Summary',
    coverImage: 'Cover Image',
    tags: 'Tags',
    tagsPlaceholder: 'Add tags (comma separated)',
    commonTags: 'Common tags (click to add/remove)',
    publishNow: 'Publish Now',
    cancel: 'Cancel',
    uploading: 'Uploading...',
    dragDrop: 'Drag & drop image or click to upload',
    remove: 'Remove',
    requiredTitle: 'Please enter a title',
    requiredContent: 'Please enter some content',
    draftSaved: 'Draft saved',
    publishSuccess: 'Article published successfully!',
    publishFailed: 'Failed to publish: {msg}',
    saveDraftFailed: 'Failed to save draft: {msg}',
    updateSuccess: 'Article updated successfully!',
    columnSelect: 'Series (up to 3)',
    columnLimit: 'An article can join up to 3 series',
    columnEmpty: 'No series yet. ',
    columnEmptyCreate: 'Create one in Creator Center',
    columnLoadFailed: 'Failed to load your series. ',
    columnLoadRetry: 'Retry',
  },
  creator: {
    title: 'Creator Center',
    createArticle: 'Write Article',
    contentManagement: 'Content Management',
    published: 'Published',
    drafts: 'Drafts',
    time: 'Published',
    views: 'Views',
    comments: 'Comments',
    actions: 'Actions',
    edit: 'Edit',
    delete: 'Delete',
    continueEdit: 'Continue',
    publish: 'Publish',
    lastModified: 'Last Modified',
    deleteConfirm: 'Delete "{title}"? This cannot be undone.',
    publishConfirm: 'Publish "{title}"?',
    deleteSuccess: 'Deleted successfully',
    publishSuccess: 'Published successfully',
    status: 'Status',
    reviewing: 'In Review',
    reviewingAll: 'All',
    reviewingAi: 'AI Review',
    reviewingManual: 'Pending Manual Review',
    reviewingRejected: 'Rejected',
    emptyReviewing: 'No articles under review yet.',
    reviewReasonTitle: 'Review Info',
    reviewReason: 'Review Reason',
    noReviewReason: 'No review reason provided.',
    emptyPublished: 'No published articles yet. Click "Write Article" to start.',
    emptyDrafts: 'No drafts yet.',
    prev: 'Prev',
    next: 'Next',
    title_column: 'Title',
    columns: 'Columns',
    newColumn: 'New Column',
    editColumn: 'Edit Column',
    save: 'Save',
    cancel: 'Cancel',
    columnName: 'Column name',
    columnNamePlaceholder: 'Give the column a name, e.g. Java Backend in Practice',
    columnDescription: 'Description (optional)',
    columnDescriptionPlaceholder: 'A sentence about what this column covers',
    columnCover: 'Cover image (optional)',
    columnNameRequired: 'Column name is required',
    columnArticlesCount: '{count} published',
    emptyColumns: 'No columns created yet.',
    columnCreated: 'Column created',
    columnUpdated: 'Column updated',
    deleteColumnConfirm: 'Delete column "{name}"? Articles in it will be unlinked (the articles themselves are kept).',
    // Column owner edit page (Task 19)
    backToColumns: 'Back to columns',
    columnMembers: 'Column articles',
    addArticles: 'Add Articles',
    removeMember: 'Remove',
    removeMemberConfirm: 'Remove "{title}" from this column?',
    moveUp: 'Move up',
    moveDown: 'Move down',
    emptyColumnMembers: 'No articles yet. Click "Add Articles" to include your published ones.',
    columnLoadFailed: 'Failed to load column',
    columnRetry: 'Retry',
    columnForbidden: 'This column does not exist or you do not have permission to edit it',
    archived: 'Archived',
    addModalTitle: 'Add my published articles',
    addModalEmpty: 'No published articles to add.',
    addModalLoadFailed: 'Failed to load candidate articles',
    addModalPageEmpty: 'Nothing addable on this page — try another page',
    addModalSelected: '{count} selected',
    addModalSubmit: 'Add',
    addResultTitle: 'Add result',
    addResultAdded: '{count} added',
    skippedHeading: 'The following could not be added:',
    noneAdded: 'None were added (they may already be in this column)',
    done: 'Done',
    skipCode: {
      limit3: 'Already in 3 columns, not added',
      not_published: 'This article is not published yet',
      not_owner: 'This is not one of your articles',
      not_found: 'This article does not exist or has been deleted',
    },
  },
  articleDetail: {
    views: 'views',
    likes: 'likes',
    edit: 'Edit',
    comments: 'Comments',
    reply: 'Reply',
    loginToComment: 'Please login to comment',
    submitComment: 'Submit',
    loadMore: 'Load More',
    noContent: 'No content available.',
    loading: 'Loading content...',
    toc: 'Table of Contents',
    related: 'Related Articles',
    share: 'Share',
    copyLink: 'Copy Link',
    linkCopied: 'Link copied to clipboard!',
    author: 'Author',
    publishedAt: 'Published at',
    updatedAt: 'Updated at',
    series: 'Series',
    columnArticles: '{count} articles',
    copied: 'Copied to clipboard successfully!',
    copyFailed: 'Failed to copy, please share manually',
    notFound: 'Article not found.',
    minRead: 'min read',
    loadFailed: 'Failed to load content.',
    previous: 'Previous',
    next: 'Next',
    loginToVote: 'Please login to vote.',
    loginToReply: 'Please login to reply.',
    loginToLike: 'Please login to like.',
    loginToFavorite: 'Please login to favorite.',
    loadingReplies: 'Loading replies...',
    addReply: 'Add a reply...',
    post: 'Post',
    report: 'Report',
    copy: 'Copy',
    cancel: 'Cancel',
    sortByHot: 'Hot',
    sortByNewest: 'Newest',
    loadMoreComments: 'Load more comments',
  },
  columnDetail: {
    noArticles: 'No articles yet',
    articleCount: '{count} articles',
  },
  cookie: {
    title: 'Privacy & Data Protection'
  },
  coverUpload: {
    dragDrop: 'Click or drag to upload a cover',
    change: 'Click to change cover',
    remove: 'Remove cover',
    uploading: 'Uploading...',
    invalidType: 'Only image files are allowed',
  },
  search: {
    filter: 'Filters',
    filterAll: 'All',
    filterArticle: 'Article',
    filterTag: 'Tag',
    filterAuthor: 'Author',
    hotSearches: 'Trending',
    searchPlaceholder: 'Type to search...',
    searching: 'Searching...',
    sort: 'Sort',
    sortRelevance: 'Relevance',
    sortNewest: 'Newest',
    sortMostLiked: 'Most Liked',
    sortMostViewed: 'Most Viewed',
    dateFrom: 'From',
    dateTo: 'To',
    reset: 'Reset',
    resultsFor: 'Found {count} results for "{keyword}"',
    resultsForTag: 'Found {count} results tagged "{keyword}"',
    resultsForAuthor: 'Found {count} results by "{keyword}"',
    resultsAll: 'Found {count} results',
    noResults: 'No results found',
    noResultsHint: 'Try different keywords or filters.',
    readyToExplore: 'Ready to explore?',
    readyToExploreHint: 'Select a topic from the sidebar or type to search.',
  },
  articles: {
    1: {
      title: 'Mastering Vue 3 Composition API',
      summary: 'Deep dive into the new Composition API features, including reactive refs, computed properties, and lifecycle hooks.',
    },
    2: {
      title: 'Spring Boot 3.0 Migration Guide',
      summary: 'A comprehensive guide on how to migrate your existing Spring Boot 2.x applications to the latest Spring Boot 3.0 version.',
    },
    3: {
      title: 'Building Microservices with Kubernetes',
      summary: 'Learn how to deploy, scale, and manage your microservices architecture using Kubernetes orchestration.',
    },
    4: {
      title: 'TypeScript Best Practices for 2024',
      summary: 'Essential TypeScript patterns and practices to write cleaner, safer, and more maintainable code.',
    }
  }
}
